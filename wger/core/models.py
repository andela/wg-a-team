...
class UserProfile(models.Model):
    ...
    show_comments = models.BooleanField(
        verbose_name=_('Show exercise comments'), help_text=_(
            'Check to show exercise comments on the '
            'workout view'), default=True)
    '''
    ...
    workout_reminder_active = models.BooleanField(
        verbose_name=_('Activate workout reminders'), help_text=_(
    ...
    workout_reminder = IntegerField(
        verbose_name=_('Remind before expiration'), help_text=_(
            'The number of days you want to be reminded '
            'before a workout expires.'), default=14, validators=[
            MinValueValidator(1), MaxValueValidator(30)])
    workout_duration = IntegerField(
        verbose_name=_('Default duration of workouts'), help_text=_(
            'Default duration in weeks of workouts not '
            'in a schedule. Used for email workout '
            'reminders.'), default=12, validators=[
            MinValueValidator(1), MaxValueValidator(30)])
    ...
    notification_language = models.ForeignKey(
        Language, verbose_name=_('Notification language'), help_text=_(
            'Language to use when sending you email '
            'notifications, e.g. email reminders for '
            'workouts. This does not affect the '
            'language used on the website.'), default=2)

    timer_active = models.BooleanField(
        verbose_name=_('Use pauses in workout timer'), help_text=_(
            'Check to activate timer pauses between '
            'exercises.'), default=True)
    '''
    Switch to activate pauses in the gym view
    '''

    timer_pause = IntegerField(
        verbose_name=_('Default duration of workout pauses'), help_text=_(
            'Default duration in seconds of pauses used by '
            'the timer in the gym mode.'), default=90, validators=[
            MinValueValidator(10), MaxValueValidator(400)])
    '''
    ...
    def clean(self):
        ...
        '''
        if ((
                self.sleep_hours and
                self.freetime_hours and
                self.work_hours) and (
                self.sleep_hours +
                self.freetime_hours +
                self.work_hours) > 24):
            raise ValidationError(
                _('The sum of all hours has to be 24'))
    ...
    def user_bodyweight(self, weight):
        ...
        '''
        if (
            not WeightEntry.objects.filter(
                user=self.user).exists() or (
                datetime.date.today() -
                WeightEntry.objects.filter(
                user=self.user).latest().date > datetime.timedelta(
                    days=3))):
            entry = WeightEntry()
            entry.weight = weight
...
class License(models.Model):
    ...
    full_name = models.CharField(
        max_length=60, verbose_name=_('Full name'), help_text=_(
    ...
    url = models.URLField(verbose_name=_('Link'), help_text=_(
        'Link to license text or other information'), blank=True, null=True)
