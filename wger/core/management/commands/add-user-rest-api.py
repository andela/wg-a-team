    ...
    def handle (self, *args, **options): 
        ...
        try:
            ...
            if options['disable']:
                ...
                self.stdout.write(self.style.SUCCESS("{0} is 'DISABLED' from adding users via the API".format(options['name'])))

            else:
                profile.can_add_user = True
                self.stdout.write(self.style.SUCCESS("{0} can add users via the API".format(options['name'])))
            profile.save()
