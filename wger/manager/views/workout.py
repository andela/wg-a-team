...
def view(request, pk):
    ...
    # Create the backgrounds that show what muscles the workout will
    # work on
    muscles_front = []
    muscles_back = []
...
def copy_workout(request, pk):
    ...
    if request.method == 'POST':
        ...
        template_data['extend_template'] = \
            'base_empty.html' if request.is_ajax(
        ) else 'base.html'
...
def timer(request, day_pk):
    ...
    day = get_object_or_404(
        Day, pk=day_pk, training__user=request.user)
    ...
    for set_dict in canonical_day['set_list']:
        ...
        # Supersets need extra work to group the exercises and reps
        # together
        else:
            total_reps = len(
                set_dict['exercise_list'][0]['reps_list'])
    ...
    # because the user didn't add any repetitions to any exercise, do
    # nothing
    try:
        step_list.pop()
    except IndexError:
    ...
    # Go through the page list and calculate the correct value for
    # step_percent
    for i, s in enumerate(step_list):
        step_list[i]['step_percent'] = (
            i + 1) * 100.0 / len(step_list)
