...
def create(request, day_pk):
    ...
    if request.method == "POST":
            ...
            for exercise in form.cleaned_data['exercises']:
                formset = SettingFormset(
                    ...
                    prefix='exercise{0}'.format(
                        exercise.id))
                formsets.append(
                    {'exercise': exercise, 'formset': formset})
        all_valid = True
        ...
        if form.is_valid() and all_valid:
            # Manually take care of the order, TODO: better move this
            # to the model
    ...
    if request.method == "POST":
        ...
        if all_valid:
            for formset in formsets:
                instances = formset['formset'].save(commit=False)
                for instance in instances:
                    # If the setting has already a set, we are
                    # editing...
                    try:
                        instance.set
