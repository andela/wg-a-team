...
def add(request, pk):
    ...
    # specific form to each exercise: the entries for weight
    # and repetitions have no indicator to which exercise they
    # belong besides the form-ID, from Django's formset
    counter = 0
    total_sets = 0
    exercise_list = {}
    form_to_exercise = {}
    ...
    for exercise_set in day.set_set.all():
        for exercise in exercise_set.exercises.all():
            ...
            # Add to list
            exercise_list[exercise.id] = {'obj': exercise, 'sets': int(
                exercise_set.sets), 'form_ids': form_id_range}
    ...
    # Define the formset here because now we know the value to pass to
    # 'extra'
    ...
    if request.method == 'POST':
        ...
        # The reason for this is that the form expects a
        # value for the exercise which is not present in
        ...
        # If all the data is valid, save and redirect to log overview
        # page
        if (
            dateform.is_valid() and
            session_form.is_valid() and
            formset.is_valid()
        ):
            ...
            # Save the Workout Session only if there is not already
            # one for this date
            ...
            instances = [
                i for i in formset.save(
                    commit=False) if i.reps]
        ...
        # Depending on whether there is already a workout session for
        # today, update the current one or create a new one (this will
        # be the most usual case)
        if WorkoutSession.objects.filter(
                user=request.user,
        ...
        for day_list in self.object.canonical_representation['day_list']:
            ...
            for set_list in day_list['set_list']:
                ...
                for exercise_list in set_list['exercise_list']:
                    ...
                    # Filter the logs for user and exclude all
                    # units that are not weight
                    #
                    # TODO: add the repetition_unit to the filter.
                    # For some reason (bug in django? DB problems?)
                    # when adding the filter there, the execution time
                    # explodes. The weight unit filter works as expected.
                    # Also, adding the unit IDs to the exclude list also
                    # has the disadvantage that if new ones are added in a
                    ...
                    if entry_log:
                        exercise_log[exercise_list['obj'].id].append(
                            entry_log)
...
def day(request, username, year, month, day):
    ...
    try:
        ...
        return HttpResponseForbidden()
    context['logs'] = group_log_entries(
