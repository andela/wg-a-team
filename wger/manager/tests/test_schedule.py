...
class ScheduleTestCase(WorkoutManagerTestCase):
    ...
    def test_schedule_overview(self):
        ...
        for i in range(0, 3):
            self.assertFalse(
                response.context['schedules'][i].is_active)
...
class SchedulePdfExportTestCase(WorkoutManagerTestCase):
    ...
    def export_pdf_with_images_and_comments(
            self, fail=False, pdf_type="log"):
        '''
        Helper function to test exporting a workout as a pdf,
    ...
    def test_export_pdf_log_with_images_and_comments(
            self, fail=False):
        '''
        Tests exporting a workout as a pdf as the owner user
...
#   #####TABLE#####
    ...
    def test_export_pdf_table_with_images_and_comments(
            self, fail=False):
        '''
        Tests exporting a workout as a pdf as the owner user
        with images andcomments
