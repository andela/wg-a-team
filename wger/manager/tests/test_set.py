...
class SetDeleteTestCase(WorkoutManagerTestCase):
    ...
    def test_delete_set_other(self):
        '''
        Tests deleting a set from a workout as a logged user
        not owning the data
        '''
...
class TestSetOrderTestCase(WorkoutManagerTestCase):
    ...
    def add_set(self, exercises_id):
        ...
        for exercise_id in exercises_id:
            post_data['exercise{0}-TOTAL_FORMS'.format(
                exercise_id)] = nr_sets
            post_data['exercise{0}-INITIAL_FORMS'.format(
                exercise_id)] = 0
            post_data['exercise{0}-MAX_NUM_FORMS'.format(
                exercise_id)] = 1000
