...
class ExerciseIndexTestCase(WorkoutManagerTestCase):
    ...
    def test_exercise_index_non_editor(self):
        '''
        Tests the exercise overview page as a logged in
...
class ExerciseDetailTestCase(WorkoutManagerTestCase):
    ...
    def exercise_detail(self, editor=False):
        ...
        if editor:
            ...
            self.assertNotContains(
                response, 'Exercise is pending review')
        else:
    ...
    def test_exercise_detail_non_editor(self):
        '''
        Tests the exercise details page as a logged in
        user without editor rights
        '''
...
class ExercisesTestCase(WorkoutManagerTestCase):
    ...
    def add_exercise_success(self, admin=False):
        ...
        response = self.client.post(reverse('exercise:exercise:add'),
                                    {'category': 1,
                                     ...
                                     'muscles': []})
        self.assertFalse(
            response.context['form'].errors.get('muscles'))
    ...
    def search_exercise(self, fail=True):
        ...
        self.assertEqual(
            result['suggestions'][0]['data']['category'],
            'Another category')
        self.assertEqual(result['suggestions']
                         [0]['data']['image'], None)
        self.assertEqual(result['suggestions']
                         [0]['data']['image_thumbnail'], None)
...
class ExercisesCacheTestCase(WorkoutManagerTestCase):
    ...
    def test_overview_cache_update(self):
        ...
        self.assertFalse(
            cache.get(
                get_template_cache_name(
                    'muscle-overview-mobile',
                    2)))
        self.assertFalse(
            cache.get(
                get_template_cache_name(
                    'muscle-overview-search',
                    2)))
        self.assertFalse(
            cache.get(
                get_template_cache_name(
