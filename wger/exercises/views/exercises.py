...
def view(request, id, slug=None):
    ...
    # Create the backgrounds that show what muscles the exercise works
    # on
    backgrounds = cache.get(
        cache_mapper.get_exercise_muscle_bg_key(
            int(id)))
    if not backgrounds:
        backgrounds_back = []
        backgrounds_front = []
        ...
        for muscle in exercise.muscles.all():
            if muscle.is_front:
                backgrounds_front.append(
                    'images/muscles/main/muscle-%s.svg' %
                    muscle.id)
            else:
        ...
        for muscle in exercise.muscles_secondary.all():
            if muscle.is_front:
                backgrounds_front.append(
                    'images/muscles/secondary/muscle-%s.svg' %
                    muscle.id)
            else:
                backgrounds_back.append(
                    'images/muscles/secondary/muscle-%s.svg' %
        ...
        # behind the muscles.
        backgrounds_front.append(
            'images/muscles/muscular_system_front.svg')
        backgrounds_back.append(
            'images/muscles/muscular_system_back.svg')
...
class ExercisesEditAddView(WgerFormMixin):
    ...
    def get_form_class(self):
        ...
        # during the request have we access to the currently used
        # language. In other places Django defaults to 'en-us'.
...
class ExerciseCorrectView(
    ...
    messages = _(
        'Thank you. Once the changes are reviewed \
    ...
    def form_valid(self, form):
        ...
        message = render_to_string(
            'exercise/email_correction.tpl', context)
...
def accept(request, pk):
    ...
    messages.success(
        request,
        _('Exercise was successfully added to the general database'))
